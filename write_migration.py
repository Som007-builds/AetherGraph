...
def migrate_papers(conn):
    ...
    rows = conn.execute(
        "SELECT id, arxiv_id, title, authors, abstract, published FROM papers"
    ).fetchall()
    for row in rows:
        ...
        run_write("""
            ...
            SET p.sqlite_id = $sqlite_id, p.title = $title,
                p.authors = $authors, p.abstract = $abstract,
                p.published = $published, p.year = $year
        """, {"arxiv_id": arxiv_id, "sqlite_id": paper_id,
              "title": title or "", "authors": authors or "",
              "abstract": abstract or "", "published": published or "",
              "year": year})
...
def migrate_claims(conn):
    ...
    rows = conn.execute("""
        ...
        FROM claims cl JOIN papers p ON cl.paper_id = p.id
    """).fetchall()
    for row in rows:
        claim_id, text, section, confidence, embedding_id, paper_year, arxiv_id = row
        run_write("""
            MERGE (c:Claim {claim_id: $claim_id})
            SET c.text = $text, c.section = $section,
                c.confidence = $confidence, c.embedding_id = $embedding_id,
                c.paper_year = $paper_year
            WITH c
            MATCH (p:Paper {arxiv_id: $arxiv_id})
            MERGE (c)-[:EXTRACTED_FROM]->(p)
        """, {"claim_id": claim_id, "text": text or "",
              "section": section or "", "confidence": confidence or 1.0,
              "embedding_id": embedding_id or "", "paper_year": paper_year,
              "arxiv_id": arxiv_id})
...
def migrate_relationships(conn):
    ...
    rows = conn.execute("""
        SELECT r.id, r.claim_a_id, r.claim_b_id, r.rel_type,
               r.explanation, r.confidence FROM relationships r
    """).fetchall()
    contradicts = 0
    supports = 0
    for row in rows:
        ...
        if rel_type == "CONTRADICTS":
            run_write("""
                ...
                SET r.explanation = $explanation,
                    r.confidence = $confidence, r.sqlite_id = $rel_id
            """, {"claim_a_id": claim_a_id, "claim_b_id": claim_b_id,
                  "explanation": explanation or "", "confidence": confidence or 0.0,
                  "rel_id": rel_id})
            contradicts += 1
            ...
            """, {"claim_a_id": claim_a_id, "claim_b_id": claim_b_id,
                  ...
                  "rel_id": rel_id})
            supports += 1
    print(f"  Migrated {contradicts} CONTRADICTS, {supports} SUPPORTS.")


def migrate_gaps(conn):
    print("\\n[4/4] Migrating gaps...")
    rows = conn.execute(
        "SELECT id, text, source, related_claim_ids FROM gaps"
    ).fetchall()
    for row in rows:
        gap_id, text, source, related_claim_ids_str = row
        run_write("""
            MERGE (g:Gap {gap_id: $gap_id})
            SET g.text = $text, g.source = $source
        """, {"gap_id": gap_id, "text": text or "", "source": source or ""})
...
with open('graph/migrate_to_neo4j.py', 'w', encoding='utf-8') as f:
    f.write(content)
