...
def insert_claim(paper_id, claim_text, section, confidence=1.0, embedding_id=None) -> int:
    conn = get_conn()
    c = conn.cursor()
    try:
        c.execute("""
            INSERT INTO claims (paper_id, claim_text, section, confidence, embedding_id)
            VALUES (?, ?, ?, ?, ?)
        """, (paper_id, claim_text, section, confidence, embedding_id))
        conn.commit()
        return c.lastrowid
    finally:
        conn.close()
...
def get_all_claims() -> list[dict]:
    ...
    try:
        c.execute("""
            SELECT cl.id, cl.claim_text, cl.section, cl.confidence, p.arxiv_id, p.title
            FROM claims cl
        ...
        return [
            {"id": r[0], "text": r[1], "section": r[2],
             "confidence": r[3], "arxiv_id": r[4], "paper_title": r[5]}
            for r in rows
        ]
    finally:
        conn.close()
